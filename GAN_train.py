...
def train(mnist):
    ...
    # 调用生成模型生成图像G_output
    G_logits, G_output = generatorModel(fake_images, UINTS_SIZE, image_size)
    # D对真实图像的判别
    real_logits, real_output = discriminatorModel(real_images, UINTS_SIZE)
    # D对G生成的图像进行判别
    fake_logits, fake_output = discriminatorModel(G_output, UINTS_SIZE, reuse=True)
    # 计算损失函数
    ...
    with tf.Session() as session:
        session.run(tf.global_variables_initializer())  # 初始化全部变量
        for epoch in range(EPOCH):
            for batch_i in range(mnist.train.num_examples // BATCH_SIZE):
                batch_image, _ = mnist.train.next_batch(BATCH_SIZE)
                # 对图像进行scale，tanh的输出结果是(-1,1)
